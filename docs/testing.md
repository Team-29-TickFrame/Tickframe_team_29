...
Latest local verification: 2026-07-10 using
...
| [`backend/app/auth.py`](../backend/app/auth.py) | Handles registration, login, password hashing, and token-based user lookup. | 30% | 71% | Local coverage run and [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |
| [`backend/app/exchanges/binance.py`](../backend/app/exchanges/binance.py) | Parses Binance trade messages and normalizes exchange data. | 30% | 44% | Local coverage run and [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |
| [`backend/app/exchanges/bybit.py`](../backend/app/exchanges/bybit.py) | Parses Bybit trade messages and manages Bybit subscriptions and rejected topics. | 30% | 74% | Local coverage run and [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |
...
| [`backend/app/pattern_ml.py`](../backend/app/pattern_ml.py) | Loads the maintained pattern-recognition artifact and returns safe prediction statuses. | 30% | 96% | Local coverage run and [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |
| [`ml/pattern_recognition/dataset.py`](../ml/pattern_recognition/dataset.py) | Loads and validates reproducible training and test windows for the baseline model. | 30% | 88% | Local coverage run and [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |
| [`ml/pattern_recognition/features.py`](../ml/pattern_recognition/features.py) | Extracts model features used by the pattern detector. | 30% | 91% | Local coverage run and [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |
| [`ml/pattern_recognition/model.py`](../ml/pattern_recognition/model.py) | Contains the baseline model logic used by the product pattern endpoint. | 30% | 76% | Local coverage run and [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |

The global measured coverage for `backend.app` and `ml.pattern_recognition` is
62%. Global coverage is lower than some critical modules because migration,
...
|---|---|---|---|---|
| Unit tests | Aggregation, metrics, auth, config, exchange parsers, lifecycle behavior, observability, and ML validation/detector behavior. | `coverage run --source=backend.app,ml.pattern_recognition -m unittest discover -s backend/tests` | Passing locally on 2026-07-10; 92 tests OK. | [`backend/tests`](../backend/tests) and [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |
| Integration tests | Service history fallback, metrics snapshots, database writer shutdown, exchange subscription handling, application lifecycle cleanup, and ML API behavior. | Same backend test command. | Passing locally on 2026-07-10. | [`backend/tests/test_service_history.py`](../backend/tests/test_service_history.py), [`backend/tests/test_database_history.py`](../backend/tests/test_database_history.py), [`backend/tests/test_lifecycle.py`](../backend/tests/test_lifecycle.py), [`backend/tests/test_api_patterns.py`](../backend/tests/test_api_patterns.py) |
| Frontend runtime helper tests | WebSocket reconnect, permanent policy-close, online recovery, and cleanup behavior. | `npm test` | Passing locally on 2026-07-10; 3 tests OK. | [`frontend/src/websocket.test.ts`](../frontend/src/websocket.test.ts) and [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |
| Automated QRTs | QR-001 latency, QR-002 failure visibility, and QR-003 critical-module testability. | Backend tests and critical coverage gate. | Passing locally on 2026-07-10. | [`docs/quality-requirement-tests.md`](quality-requirement-tests.md) and [`backend/tests/test_quality_requirements.py`](../backend/tests/test_quality_requirements.py) |
| Critical-module coverage | All critical modules listed above. | `python backend/scripts/check_critical_coverage.py coverage.json` | Passing locally on 2026-07-10 for 10 modules at >= 30%. | [`backend/scripts/check_critical_coverage.py`](../backend/scripts/check_critical_coverage.py) |

## CI and QA Check Status

| Gate or check | Required for Done? | Command or CI check | Latest status | Evidence |
|---|---|---|---|---|
| Backend linting | Yes | `ruff check backend ml` | Passing locally on 2026-07-10. | [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |
| Backend formatting | Yes | `ruff format --check backend ml` | Passing locally on 2026-07-10 after applying Ruff formatting. | [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |
| Backend tests and coverage | Yes | `coverage run --source=backend.app,ml.pattern_recognition -m unittest discover -s backend/tests` | Passing locally on 2026-07-10. | [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) and `backend-coverage` artifact |
| Critical-module coverage | Yes | `python backend/scripts/check_critical_coverage.py coverage.json` | Passing locally on 2026-07-10. | [`backend/scripts/check_critical_coverage.py`](../backend/scripts/check_critical_coverage.py) |
| Frontend type checking | Yes | `npm run typecheck` | Passing locally on 2026-07-10. | [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |
| Frontend runtime helper tests | Yes | `npm test` | Passing locally on 2026-07-10; 3 tests OK. | [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |
| Frontend build | Yes | `npm run build` | Passing locally on 2026-07-10. | [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml) |
| Additional QA check | Yes | `ruff check --select DTZ backend/app ml/pattern_recognition` | Passing locally on 2026-07-10. | [Quality workflow](https://github.com/Team-29-TickFrame/Tickframe_team_29/actions/workflows/quality.yml), `Additional QA - datetime safety analysis` job |
...
- Frontend TypeScript type checking and reconnect runtime tests.
